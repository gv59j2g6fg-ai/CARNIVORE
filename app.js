...
function defaultDailyFoodRows() {
  const preferred = ["Lamb loin chops", "Scotch fillet", "T-bone steak"];
  return preferred
    .filter(name => foods.some(f => f.name === name))
...
if (!dayDraft.date || dayDraft.date !== todayISO()) {
  dayDraft = freshDayDraft(todayISO());
...
function saveWeightForDate(date, bw, bwUnit) {
  const n = Number(bw);
  if (!date) return;

  if (!isFinite(n) || n <= 0) {
    delete weightHistory[date];
  } else {
    weightHistory[date] = { bw: n, bwUnit: bwUnit || "kg" };
  }
...
function getWeightEntry(date) {
  return weightHistory[date] || null;
}

function loadWeightForSelectedDate() {
  const date = selectedDate();
  const entry = getWeightEntry(date);
  if (entry) {
    $("bw").value = String(entry.bw ?? "");
    $("bwUnit").value = entry.bwUnit || "kg";
  } else {
    $("bw").value = String(targets.bw ?? "");
...
function persistTargetsFromUI() {
  targets = readTargetsFromUI();
  saveJSON(LS.TARGETS, targets);
  computeTargetsAndUpdateUI();
}

function queueAutoSaveWeight() {
  clearTimeout(weightSaveTimer);
  weightSaveTimer = setTimeout(() => {
    const current = readTargetsFromUI();
    targets = { ...targets, bw: current.bw, bwUnit: current.bwUnit, pgkg: current.pgkg, tcal: current.tcal, tcarb: current.tcarb };
    saveJSON(LS.TARGETS, targets);
    saveWeightForDate(selectedDate(), current.bw, current.bwUnit);
    computeTargetsAndUpdateUI();
    toast("Weight auto-saved");
  }, 250);
}

function onTargetFieldChanged() {
  persistTargetsFromUI();
  toast("Targets auto-saved");
}

function renderWeightHistoryList() {
  const wrap = $("weightHistoryList");
  if (!wrap) return;
  wrap.innerHTML = "";

  const dates = Object.keys(weightHistory).sort((a,b) => b.localeCompare(a));
  if (!dates.length) {
    wrap.innerHTML = `<div class="muted">No weight entries yet.</div>`;
    return;
  }

  dates.slice(0, 12).forEach(date => {
    const item = weightHistory[date];
    const div = document.createElement("div");
    div.className = "hitem";
    div.innerHTML = `
      <div class="meta">
        <div class="d">${escapeHTML(date)}</div>
        <div class="s">${round1(item.bw)} ${escapeHTML(item.bwUnit || "kg")}</div>
      </div>
    `;
    ...
    loadBtn.addEventListener("click", () => {
      $("date").value = date;
      if (history[date]) loadDay(date);
      else openUnsavedDate(date);
...
function renderWeightGraph() {
  ...
  const entries = Object.keys(weightHistory)
    ...
    .map(date => ({ date, ...(weightHistory[date] || {}) }))
    .filter(x => isFinite(Number(x.bw)) && Number(x.bw) > 0);
  ...
  if (!entries.length) {
    ...
    ctx.fillText("No weight data yet", 16, 28);
    return;
  }
  ...
  const vals = entries.map(x => Number(x.bw));
  ...
  ctx.strokeStyle = "rgba(42,140,255,.95)";
  ctx.lineWidth = 2.5;
  ctx.beginPath();
  entries.forEach((entry, i) => {
    const x = entries.length === 1 ? padL + plotW / 2 : padL + (plotW * i / (entries.length - 1));
    const y = padT + plotH - ((Number(entry.bw) - minV) / range) * plotH;
  ...
  ctx.textAlign = "right";
  ctx.fillText(round1(maxV).toFixed(1), padL - 8, padT + 4);
  ctx.fillText(round1(minV).toFixed(1), padL - 8, padT + plotH);

  ctx.textAlign = "left";
  ctx.fillText(entries[0].date.slice(5), padL, height - 10);
  ctx.textAlign = "right";
  ctx.fillText(entries[entries.length - 1].date.slice(5), width - padR, height - 10);

  const latest = entries[entries.length - 1];
  ctx.textAlign = "left";
  ctx.fillStyle = "#e8eef7";
  ctx.fillText(`Latest: ${round1(latest.bw)} ${latest.bwUnit || "kg"}`, padL, 16);
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  $("date").addEventListener("change", () => {
    ...
    if (history[d]) loadDay(d);
    else openUnsavedDate(d);
    loadWeightForSelectedDate();
    renderWeightGraph();
  });
...
// Targets fill
  ...
  // Targets auto-save
  $("bw").addEventListener("input", queueAutoSaveWeight);
  $("bw").addEventListener("change", queueAutoSaveWeight);
  $("bw").addEventListener("blur", queueAutoSaveWeight);
  ["pgkg", "tcal", "tcarb"].forEach((id) => {
    const el = $(id);
    el.addEventListener("input", onTargetFieldChanged);
    el.addEventListener("change", onTargetFieldChanged);
    el.addEventListener("blur", onTargetFieldChanged);
  });
  $("bwUnit").addEventListener("input", () => { queueAutoSaveWeight(); });
  $("bwUnit").addEventListener("change", () => { queueAutoSaveWeight(); });

  // Food rows
  $("addFoodRow").addEventListener("click", () => addFoodRow());
  $("keepForTomorrow").addEventListener("click", keepSelectedForTomorrow);
  ...
  recalcTotals();
  renderWeightGraph();
  window.addEventListener("resize", renderWeightGraph);
  installHint();
  ...
  // Ensure daily default foods are there, but drinks stay empty until you add one
  if (!dayDraft.foodRows.length) {
    dayDraft.foodRows = defaultDailyFoodRows();
    renderFoodRows();
    persistDraft();
    recalcTotals();
  }
...
function saveCurrentDay() {
  ...
  const payload = {
    ...
    bodyWeight: Number($("bw").value) || 0,
    bodyWeightUnit: $("bwUnit").value || "kg"
  };

  saveWeightForDate(d, payload.bodyWeight, payload.bodyWeightUnit);

  history[d] = payload;
  saveJSON(LS.HISTORY, history);
  delete plannedDays[d];
...
function openUnsavedDate(date) {
  ...
  dayDraft = plan ? {
    ...
    drinkRows: (plan.drinkRows || []).map(normalizeDrinkRow)
  } : freshDayDraft(date);
  $("date").value = date;
  renderFoodRows();
  renderDrinkRows();
...
function keepSelectedForTomorrow() {
  const selected = [...document.querySelectorAll('input[data-kind="carry-food"]:checked')]
    .map(input => dayDraft.foodRows[Number(input.dataset.idx)])
    .filter(row => row && row.food)
    .map(row => ({ food: row.food, grams: Number(row.grams) || 0 }));
...
function loadDay(date) {
  ...
  if (isFinite(Number(item.bodyWeight)) && Number(item.bodyWeight) > 0) {
    weightHistory[date] = { bw: Number(item.bodyWeight), bwUnit: item.bodyWeightUnit || "kg" };
    saveJSON(LS.WEIGHT_HISTORY, weightHistory);
  }
  renderFoodRows();
  renderDrinkRows();
...
function renderFoodRows() {
  ...
  dayDraft.foodRows.forEach((row, idx) => {
    ...
    carry.type = "checkbox";
    carry.dataset.kind = "carry-food";
    carry.dataset.idx = String(idx);
    carry.title = "Keep this food for tomorrow";
    carry.setAttribute("aria-label", `Keep ${row.food || "this food"} for tomorrow`);

    const actions = document.createElement("div");
    actions.className = "carryControls";
    ...
    tr.appendChild(f);
    tr.appendChild(c);
    tr.appendChild(wrapCell(actions));
    wrap.appendChild(tr);
  });
